...
const JobseekerDashboard = () => {
    ...
    const handleDeleteApplication = async (id) => {
        ...
        try {
            await axiosInstance.delete(`/application/delete/${id}`);
                                ...
                                <div className="flex justify-between items-start">
                                    <div>
                                        <h4 className="font-bold text-gray-800 text-lg">
                                            {app.jobId?.title || app.job?.title}
                                        </h4>
                                        <p className="text-blue-600 font-medium mt-1">
                                            {app.jobId?.company || app.job?.company}
                                        </p>
                                        <div className="flex gap-3 mt-2 text-gray-500 text-sm">
                                            <span>{app.jobId?.location || app.job?.location}</span>
                                            <span>•</span>
                                            <span>{app.jobId?.jobType || app.job?.jobType}</span>
                                        </div>
                                    </div>
                                    <span
...
const RecruiterDashboard = () => {
    ...
    const fetchMyJobs = async () => {
        try {
            const res = await axiosInstance.get("/job/getmyjobs");
    ...
    const fetchApplications = async (jobId) => {
        ...
        try {
            ...
            // Filter applications for the selected job in frontend
            const jobApps = res.data.applications.filter(app => 
    ...
    const handleStatusChange = async (appId, status) => {
        try {
            await axiosInstance.put(`/application/update/${appId}`, { status });
                                ...
                                <div key={app._id} className="p-4">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            ...
                                            <p className="text-gray-600 text-sm mt-3 bg-gray-50 p-2 rounded border border-gray-100">
                                    ...
                                    {app.resume?.url && (
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noreferrer"
                                            className="text-blue-600 text-sm hover:underline mt-2 inline-block font-semibold"
                                        >
